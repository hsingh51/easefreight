...
class RouteController extends Controller
{
    ...
    public function viewAFRRoute()
    {
        ...
        $query =  DB::table('afr_routes')->leftjoin('cities as ocity', 'afr_routes.origin_city_id', '=', 'ocity.city_id')
            ...
            ->leftjoin('itinerary', 'itinerary.afr_route_rates_id', '=', 'rates.afr_route_rates_id')
    ...
    public function addOceanRoute(Request $request) {
       ...
       $post = $request->all();
        ...
        if($ocean_route_id = DB::table('ocean_routes')->insertGetId($datas)){
            ...
            }elseif(@$post['submit_add_lcl']){
                if($this->locale == "es"){
                    return Redirect::to('/es/admin/oceanLCL/Add/'.$ocean_route_id.'/0/0');
                }else{
                    return Redirect::to('/admin/oceanLCL/Add/'.$ocean_route_id.'/0/0');
                }    
            }
